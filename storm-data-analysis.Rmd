...
Preview a sample of the unique Event Types.

```{r, echo = TRUE}
stormDataTidy %>% distinct(EVTYPE) %>% sample_n(size=10) %>% arrange(EVTYPE)
```
...
```{r, echo = TRUE}
stormDataTidy$DATE_START <- as.POSIXct(stormDataTidy$BGN_DATE, format = "%Y-%m-%d")
#stormDataTidy$DATE_END <- strptime(as.character(stormDataTidy$END_DATE), "%m/%d/%Y")
...
as.POSIXct(stormData$BGN_DATE, format = "%Y-%m-%d")

Preview new date types.

```{r, echo = TRUE}
stormDataTidy %>% select(EVTYPE, DATE_START) %>% arrange(EVTYPE)
```
